...
class ExplanationDialogue:
    ...
    def _answer_challenge(self, record, node):
        ...
        if outcome == "succeeds":
            body = (
                "%s succeeds. The plan genuinely fails on this point, so the "
                "attack on the plan summary argument is not blocked.\n%s"
                % (cq, record.get("Detail", ""))
            )
            return Move("E", Locution.CONCEDE, target=node,
                        content={"record": record, "meta": meta}, text=body)

        body = ("%s does not apply here. %s No attack on the plan summary "
                "argument arises from it." % (cq, record.get("Detail", "")))
